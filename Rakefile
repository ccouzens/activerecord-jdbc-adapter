...
ADAPTERS = %w[derby h2 hsqldb mssql mysql postgresql sqlanywhere sqlite3].map { |a| "activerecord-jdbc#{a}-adapter" }
DRIVERS  = %w[derby h2 hsqldb jtds mysql postgres sqlanywhere sqlite3].map { |a| "jdbc-#{a}" }
